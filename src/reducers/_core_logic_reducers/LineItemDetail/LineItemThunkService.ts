...
} from "@/services/_core_logic_services/CoreLineItemDetailService";
...
import { claimContentListV2 } from "@/services/_core_logic_services/CoreAdjusterPropertyClaimDetailService";
...
export const fetchRetailersDetails = createAsyncThunk(
  ...
  async (_, api) => {
    ...
    try {
      ...
      const res = await getLineItemRetailers(token);
      return res;
    } catch (error) {
...
export const fetchRoom = createAsyncThunk("lineItem/room", async (claim: string, api) => {
  ...
  try {
    const state = api.getState() as RootState;
    const token = selectAccessToken(state);
    const res = await getLineItemRoom(claim, token);
    return res;
  } catch (error) {
    return rejectWithValue(error);
  }
...
export const fetchCondition = createAsyncThunk("lineItem/condition", async (_, api) => {
  ...
  try {
    const state = api.getState() as RootState;
    const token = selectAccessToken(state);
    const res = await getLineItemCondition(token);
    return res;
  } catch (error) {
    return rejectWithValue(error);
  }
...
export const fetchLineItemCatergory = createAsyncThunk(
  ...
  async (_, api) => {
    ...
    try {
      const state = api.getState() as RootState;
      const token = selectAccessToken(state);
      const res = await getLineItemCategory(token);
      return res;
    } catch (error) {
      return rejectWithValue(error);
    }
...
export const fetchSubCategory = createAsyncThunk(
  ...
  async (categoryId: number, api) => {
    ...
    try {
      ...
      const res = await getLineItemSubCategory({ categoryId }, token);
      return res;
    } catch (error) {
      return rejectWithValue(error);
    }
...
export const fetchLineItemDetail = createAsyncThunk(
  ...
  async ({ itemId, refresh = false }: { itemId: number; refresh?: boolean }, api) => {
    ...
    try {
      const state = api.getState() as RootState;
      const token = selectAccessToken(state);
      const res = await fetchClaimItemDetails({ itemId }, token);
      if (res.status === 200) {
...
export const searchComparable = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const states = api.getState() as RootState;
      const token = selectAccessToken(states);
...
export const deleteCustomItem = createAsyncThunk(
  ...
  async (comaprableItem: unknownObjectType, api) => {
    ...
    try {
      const state = api.getState() as RootState;
      const token = selectAccessToken(state);
      const res = await removeCustomComparable(comaprableItem.id, token);
      if (res.status === 200) {
...
export const removeAttachment = createAsyncThunk(
  ...
  async (payload: { id: number; callback: () => void }, api) => {
    ...
    try {
      const state = api.getState() as RootState;
      const token = selectAccessToken(state);
      const res = await deleteAttachment({ id, token });
      callback();
      if (res?.status === 200) {
        dispatch(
          addNotification({
...
export const saveComparable = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const states = api.getState() as RootState;
      const token = selectAccessToken(states);
      const res = await handleLineItemSave(param, token);
      callback && callback();
      if (res.status === 200) {
        dispatch(
          addNotification({
...
export const fetchClaimContentAction = createAsyncThunk(
  ...
  async (payload: { claimId: string }, api) => {
    ...
    try {
      ...
      const res = await claimContentListV2(payload, token);
      return res;
    } catch (error) {
      return rejectWithValue(error);
    }
