...
import { claimContentListV2 } from "@/services/_adjuster_services/ClaimContentListService";
import { createAsyncThunk, createSlice } from "@reduxjs/toolkit";

export const initialClaimContentState: unknownObjectType = {
...
const ClaimContentSlice = createSlice({
  initialState: initialClaimContentState,
  name: EnumStoreSlice.CLAIM_CONTENT_DATA,
  reducers: {
    addClaimContentListData(state, action) {
      ...
      if (claimContentData.data) {
        ...
        state.claimContentListDataFull = [];
