...
const ClaimedItemsComponent: React.FC<connectorType> = ({
  addClaimedItemsListData,
  token,
  receiptMapperDate,
}) => {
  ...
  const getItems = React.useCallback(async () => {
    ...
    const reciptResp = await getReceiptMapperDate(
      {
        claimId: claimId,
      },
      token
    );
    if (reciptResp.status === 200) {
      receiptMapperDate({ receiptMapperPdf: reciptResp.data });
    }
...
export default connector(ClaimedItemsComponent);
