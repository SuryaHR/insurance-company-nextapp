...
const SummaryComponent: React.FC<connectorType & typeProps> = (props) => {
  ...
  const handleDownloadSummaryPDF = async () => {
    setTableLoader(true);
    const status = await exportSummaryToPDF(claimNumber, token);
    if (status === "success") {
      addNotification({
...
const mapStateToProps = (state: any) => ({
  ...
  totalCashPaid: state.claimedItems.totalCashPaid,
  noOfHoldoverItems: state.claimedItems.noOfHoldoverItems,
  TotalHoldoverPaid: state.claimedItems.TotalHoldoverPaid,
  TotalHoldoverDue: state.claimedItems.TotalHoldoverDue,
