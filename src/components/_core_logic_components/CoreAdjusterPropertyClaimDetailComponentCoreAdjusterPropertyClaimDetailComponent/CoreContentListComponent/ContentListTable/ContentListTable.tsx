...
const ContentListTable: React.FC<connectorType & typeProps> = (props) => {
  ...
  const editAction = async (rowData: any) => {
    ...
    const result = await fetchClaimContentItemDetails(
      payload,
      claimContentListData,
      token
    );
    addEditItemDetail(result);
    setEditItem(rowData);
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    setDelete(null);
    const res = await deleteClaimItem(deletePayload, token);
    deleteClaimContentListItem({ itemId: id });
  ...
  const handleRowClick = (rowData: any) => {
    router.push(
      `/core-logic/core-adjuster-line-item-detail/${claimId}/${rowData.itemId}`
    );
  };
    ...
    <>
      ...
      <div className={ContentListTableStyle.claimTableContainer}>
        <CustomReactTable
          ...
          tableCustomClass={ContentListTableStyle.coreLogicTable}
