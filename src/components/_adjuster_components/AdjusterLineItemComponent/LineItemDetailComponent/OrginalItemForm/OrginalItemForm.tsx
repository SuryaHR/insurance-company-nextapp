...
import selectAttachment from "@/reducers/_adjuster_reducers/LineItemDetail/Selectors/selectAttachment";
...
const OrginalItemForm: React.FC<propType & connectorType> = (props) => {
  ...
  const scheduleAmountRef = useRef<currencyFormatHandlers>(null);
  useEffect(() => {
    if (isDataUpdated) {
      insuredPriceRef?.current?.changeValue(
        getUSDCurrency(lineItem?.insuredPrice).toString()
      );
      scheduleAmountRef?.current?.changeValue(
        getUSDCurrency(lineItem?.scheduleAmount).toString()
      );
      setValue("description", lineItem?.description);
      setValue("insuredPrice", lineItem?.insuredPrice);
      setValue("scheduleAmount", lineItem?.scheduleAmount);
      setValue("quantity", lineItem?.quantity);
      setValue("ageYears", lineItem?.ageYears);
      setValue("ageMonths", lineItem?.ageMonths);
      setValue("brand", lineItem?.brand);
      setValue("model", lineItem?.model);
    }
  }, [isDataUpdated, setValue, lineItem]);
          ...
          <div className={orginalItemFormStyle.formGroup}>
            ...
            <GenericUseFormInput
              ...
              onChange={(e: React.FocusEvent<HTMLInputElement>) => {
                ...
                quantityOnChange(e);
                debounce({ quantity: e.target.value });
                setIsPageUpdated(true);
              }}
              isNumberOnly={true}
              {...quantityRegister}
