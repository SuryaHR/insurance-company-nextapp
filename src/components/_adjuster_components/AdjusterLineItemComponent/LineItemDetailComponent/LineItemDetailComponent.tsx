...
import { addtoComparableList } from "@/reducers/_adjuster_reducers/LineItemDetail/LineItemThunkService";
import { capitalize } from "lodash";
import { reviewItemSupervisor } from "@/services/_adjuster_services/AdjusterPropertyClaimDetailServices/AdjusterPropertyClaimDetailService";
import { LineItemContext } from "../LineItemContext";
import selectItemCategory from "@/reducers/_adjuster_reducers/LineItemDetail/Selectors/selectItemCategory";
import { addMessage } from "@/services/_adjuster_services/CommonDataServices/CommonDataSerive";
import { updateLineItemStatus } from "@/reducers/_adjuster_reducers/LineItemDetail/LineItemDetailSlice";
import { getParticipantsList } from "@/services/_adjuster_services/AdjusterMyClaimServices/LineItemDetailService";
...
function LineItemDetailComponentForm() {
  ...
  const { register, handleSubmit, control, getValues, setValue } = useCustomForm(
    schema,
    defaultValue
  );
  ...
  const fetchParticipants = useCallback(async () => {
    try {
      const res = await getParticipantsList(+itemId);
      if (res?.status === 200) {
  ...
  const handleAcceptStandardCost = () => {
    if (!lineItem.standardCost || lineItem.standardCost <= 0) {
      dispatch(
        addNotification({
          message: "Standard cost not available for this item",
          id: new Date().valueOf(),
          status: "warn",
        })
      );
    ...
    <>
      ...
      >
        ...
        <GroupedActionButtons
          handleAcceptStandardCost={handleAcceptStandardCost}
          onDeleteClick={showDeleteModal}
          onSupervisorReviewClick={toggleSupervisorModal}
          showAcceptStandardCost={
            lineItem.standardCost == null || lineItem.standardCost == "0"
          }
        />
