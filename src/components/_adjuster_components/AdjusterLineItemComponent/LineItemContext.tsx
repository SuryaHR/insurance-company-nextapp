...
export const LineItemContextProvider: React.FC<IFilesProviderProps> = ({ children }) => {
  ...
  const dataSaveTimeout = useRef<ReturnType<typeof setInterval> | null>(null);
  const handleDataSaveUpdate = () => {
    if (dataSaveTimeout.current) {
      clearTimeout(dataSaveTimeout?.current);
    }
    setIsDataUpdated(true);
    dataSaveTimeout.current = setTimeout(() => {
