...
import ClaimReportsTableComponentStyle from "./ClaimReportsTableComponent.module.scss";
...
const ClaimReportsTableComponent: React.FC<typeProps> = (props) => {
  const {
    tableLoader,
    resetPagination,
    setResetPagination,
  ...
  const columns = [
    columnHelper.accessor("claimNumber", {
      ...
      cell: ({ row, renderValue }) => {
        return (
          <>
            <span className={ClaimReportsTableComponentStyle.expand}>
              {renderValue()}
            </span>
            {row.getCanExpand() ? (
              row.getIsExpanded() ? (
  ...
  }, [claimsforReportList?.currentPageNumber, claimsforReportList?.claims]);
  ...
  return (
    <div className={ClaimReportsTableComponentStyle.claimTableContainer}>
