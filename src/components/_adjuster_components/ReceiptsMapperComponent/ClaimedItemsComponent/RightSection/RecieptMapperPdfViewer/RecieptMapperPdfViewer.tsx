...
const RecieptMapperPdfViewer: React.FC<connectorType & typeProps> = ({
  ...
  const handleDelete = async () => {
    ...
    if (itemData[0]?.replaceItems) {
      ...
      const result = await deleteMappedLineItem(payload);
      if (result) {
        getClaimedItems({
          claimId: claimId,
      ...
      <div className="p-3">
        ...
        <div className={receiptMapperStyle.pdfRelativeContainer}>
          <PdfViewer
            ...
            scale={scale}
            pdfCustomClassname={clsx({
              [receiptMapperStyle.receiptPdfPagesCursor]: !modalIsOpen,
              [receiptMapperStyle.receiptPdfPages]: modalIsOpen,
            })}
