...
const MapperPopup: React.FC<connectorType & typeProps> = ({
  ...
  const saleTaxRate = sessionStorage.getItem("taxRate");
  ...
  const getItemData = (value: any) => {
    ...
    if (isEmpty(itemData)) {
      ...
      const items = {
        ...
        status: itemData[0].status?.status,
        subRowIndex: index,
      };
  ...
  const submitForm = async (data: Output<typeof schema>) => {
    ...
    if (validateForm(null, data)) {
      ...
      const payload = {
        acv: itemDataArr?.averageRCVQuantity * data.quantity,
      ...
      if (itemDataArr.editId) {
        result = await updateMappedLineItem(payload);
      } else {
        result = await addAttachitemreplacementcost(payload);
      }
      if (result) {
        closeModal();
        getClaimedItems({
            ...
            <GenericUseFormInput
              ...
              onFocus={(e: any) => {
                if (
                  e.target.value === "" &&
                  !isEmpty(itemDataArr) &&
                  itemDataArr.status === "PARTIAL REPLACED"
                ) {
                  setValue("quantity", itemDataArr.remainingQuantity);
                }
              }}
...
const mapStateToProps = ({ receiptMapper, claimedItems }: any) => ({
  selectedPdf: receiptMapper.selectedPdf,
  selectedMappedItem: receiptMapper.selectedMappedItem,
